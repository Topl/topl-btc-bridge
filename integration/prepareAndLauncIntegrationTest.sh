...
export CONTAINER_ID=`docker run --rm -d --name bifrost01 -p 9085:9085 -p 9084:9084 -p 9091:9091 -v $(pwd)/node01:/bifrost-staking:rw ghcr.io/topl/bifrost-node:2.0.0-beta3-29-11442de4 --  --config  /bifrost-staking/config.yaml --regtest`
...
docker run --rm -d --name bifrost02 -e BIFROST_P2P_KNOWN_PEERS=$IP_CONTAINER:9085 -p 9087:9085 -p 9086:9084 -p 9092:9091 -v $(pwd)/node02:/bifrost-staking:rw ghcr.io/topl/bifrost-node:2.0.0-beta3-29-11442de4 --  --config  /bifrost-staking/config.yaml --regtest
